...
async function fetchTikTok() {
  ...
  try {
    ...
    if (data.status) {
      ...
      const imageLinks = data.result.download; // Assuming this contains the image URLs
      ...
      if (imageLinks && imageLinks.length > 0) {
        ...
        imageLinks.forEach((link, index) => {
          imageButtonsHTML += `<button onclick="downloadImage(${index})" class="image-download-btn">Download Foto ${index + 1}</button><br>`;
        });
        mediaPreview.innerHTML = imageButtonsHTML;
      ...
      window.imageDownloadLinks = imageLinks;

      // Auto clear the input after showing the result
      clearInput('tiktok-url');
...
function downloadImage(index) {
  const imageLink = window.imageDownloadLinks[index];
  ...
  fetch(imageLink)
    ...
    .then(blob => {
      ...
      a.href = url;
      a.download = filename;  // Set the filename to the image filename
...
async function fetchInstagram() {
  ...
  try {
    ...
    if (data.status) {
      ...
      const musicLink = data.result.media.music ? data.result.media.music : null;
      ...
      // Enable buttons based on available media
      document.getElementById('download-video-btn').disabled = !videoLink;
      document.getElementById('download-image-btn').disabled = !imageLink;
      document.getElementById('download-music-btn').disabled = !musicLink;

      window.videoDownloadLink = videoLink;
      window.imageDownloadLink = imageLink;
      window.musicDownloadLink = musicLink;

      document.getElementById('result').style.display = 'block';

      // Auto clear the input after showing the result
      clearInput('instagram-url');
...
function generateRandomFileName(extension) {
  ...
  const randomSuffix = Math.random().toString(36).substr(2, 5);  // Random string
  return `file_${timestamp}_${randomSuffix}.${extension}`;  // Randomized filename
}

function autoDownload() {
  const selectedType = document.getElementById('download-select').value;
  let downloadLink = '';
  let filename = '';
  ...
  if (selectedType === 'video') {
    ...
    filename = generateRandomFileName('mp3');
  }
