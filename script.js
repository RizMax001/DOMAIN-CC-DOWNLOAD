...
function renderResult(result, originalUrl) {
  ...
  if (vUrl) {
    ...
    videoEl.style.cssText = 'width:100%;max-height:520px;border-radius:12px;background:#000;display:block;object-fit:contain;';
  ...
  // Caption: jangan tampilkan kalau isinya cuma URL / link mentah
  const captionEl = document.getElementById('caption');
  const looksLikeUrl = /^https?:\/\//i.test(caption.trim()) || /tiktok\.com|instagram\.com|youtu|facebook\.com|twitter\.com|x\.com/i.test(caption);
  const cleanCaption = caption.trim();
  if (!cleanCaption || looksLikeUrl) {
    captionEl.textContent = '';
    captionEl.style.display = 'none';
  } else {
    captionEl.style.display = '';
    captionEl.textContent = cleanCaption.length > 130 ? cleanCaption.substring(0, 130) + '…' : cleanCaption;
  }
...
/* ============================================================
   PREMIUM ENHANCEMENT LAYER (added)
   Scroll progress · Back-to-top · Reveal-on-scroll ·
   Hero spotlight · 3D tilt cards
   ============================================================ */
(function premiumEnhance() {
  ...
  if (!reduceMotion && window.matchMedia('(pointer:fine)').matches) {
    const tiltCards = document.querySelectorAll('.plat-card, .feat-card');
  ...
  console.log('%c✨ SnapDrop Premium Enhancements Loaded', 'color:#22d3ee; font-weight:bold;');
})();
